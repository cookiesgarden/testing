...
;( function( window ) {
	...
	function Photostack( el, options ) {
  	...
  	extend( this.options, options );
		...
		this.current = getRandomInt(this.itemsCount);
        this._init();
		var ps = this;
	...
	Photostack.prototype._open = function( beforeStep ) {
		...
		var setTransition = function() { 
			if( support.transitions ) {
                el.classList.add("photostack-transition");
				//classie.addClass( el, 'photostack-transition' ); 
			}
		}
		if( beforeStep ) {
            el.classList.remove("photostack-start");
    ...
    Photostack.prototype._showMessage = function () {
        return;
    }
			...
			moveItems = function() {
				...
				self.allItems.forEach( function( item, i ) {
					...
					if(self.items.indexOf(item) === self.current && self.started && iter === 0) {
                        if (ele.options.timer == 4) {
                            let msg = document.createElement( 'div' );
                            let content = document.createTextNode("Go to intranet for more pictures!"); 
                            msg.setAttribute("id", "message");
                            msg.appendChild(content); 
                            console.log(ele);
                            document.getElementById('test').appendChild( msg );
                        }
                        else {
